   ...
   while(length > 0)
   {
      ...
      if(tag.constructed)
      {
         ...
         else
         {
            //If the maximum level of recursion is reached, then dump contents
			 printf("");
			// printf_ARRAY(prefix[level + 1], tag.value, tag.length);
         }
      }
      //Primitive type?
      else
      {
         ...
         case ASN1_TYPE_OBJECT_IDENTIFIER:
            //Append prefix
            printf(prefix[level + 1]);
            //Print OID
            printf("%s", oidToString(tag.value, tag.length, NULL, 0));
            //Add a line feed
            printf("\r\n");
            break;
         //String?
         ...
         case ASN1_TYPE_BMP_STRING:
            //Append prefix
            printf("%s", prefix[level + 1]);
            //Dump the entire string
            for(i = 0; i < tag.length; i++)
               printf("%c", tag.value[i]);
            //Add a line feed
            printf("\r\n");
            break;
         //UTC time?
         case ASN1_TYPE_UTC_TIME:
            ...
            printf("%s", prefix[level + 1]);
            //Display date
            printf("%c%c/%c%c/%c%c ", tag.value[0], tag.value[1],
               tag.value[2], tag.value[3], tag.value[4], tag.value[5]);
            //Display time
            printf("%c%c:%c%c:%c%c", tag.value[6], tag.value[7],
         ...
         case ASN1_TYPE_GENERALIZED_TIME:
            ...
            printf("%s", prefix[level + 1]);
            //Display date
            printf("%c%c%c%c/%c%c/%c%c ", tag.value[0], tag.value[1], tag.value[2],
               tag.value[3], tag.value[4], tag.value[5], tag.value[6], tag.value[7]);
            //Display time
            printf("%c%c:%c%c:%c%c", tag.value[8], tag.value[9],
         ...
         default:
			...
			printf("array");
           // TRACE_ARRAY(prefix[level + 1], tag.value, tag.length);
            break;
         }
      }
   }

 ...
 * @return Error code
 **/
error_t pemReadRsaPublicKey(const char_t *input, size_t length, RsaPublicKey *key)
{
...
//memcpy(&(key->n), tag.value, tag.length);
//key->n.data =tag.value;
//key->n.size = tag.length;
//key->n.sign = 1;   //luke
...
//key->e.data = tag.value;
//key->e.size = tag.length;
//key->e.sign = 1;
...
error_t pemReadRsaPrivateKey(const char_t *input, size_t length, RsaPrivateKey *key)
{
   ...
   do
   {
	  ...
	  printf("  Prime 2:\r\n");
	  printf("    ", &key->q);
	  printf("  Prime exponent 1:\r\n");
	  printf("    ", &key->dp);
	  printf("  Prime exponent 2:\r\n");
	  printf("    ", &key->dq);
	  printf("  Coefficient:\r\n");
	  printf("    ", &key->qinv);
